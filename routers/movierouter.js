...
module.exports={
    ...
    deleteMovieByYear:function(req,res){
        Movie.deleteMany({}).where("year").lte(req.params.year).exec(function(err,data){
            if (!err) {
                res.json(data);
    ...
    getMovieByRating:function(req,res){
        Movie.find({},"actors").where("rating").gte(req.params.rating).populate("actors").exec(function(err,data){
            if (!err) res.json(data);
        })
    }, 
